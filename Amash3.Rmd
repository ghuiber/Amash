...
If it were true that money cannot buy influence, the graph would be a flat horizontal line at whatever the average probability of a No vote is by party: more funding won't change this probability. Wouldn't that be nice?
...
Model 2 says that Democrats are more expensive: not only are they less likely to vote No at baseline, but Democratic gains in the probability of a No vote in response to funding grow more slowly. The average difference is larger than the $30,000 estimated by Model 1 and widening. The price to flip a Republican looks to be about $15,000, and a Democrat will cost something more than $50,000. 
...
```{r getProbs30k, echo=FALSE, eval=TRUE, warning=FALSE, message=FALSE, cache=TRUE}
model1Yes <- sum(p1.No$Yes[p1.No$Amount==30000]*table(df$Party))/sum((table(df$Party)))
model1No  <- sum(p1.No$No[p1.No$Amount==30000]*table(df$Party))/sum((table(df$Party)))
model2Yes <- sum(p2.No$Yes[p2.No$Amount==30000]*table(df$Party))/sum((table(df$Party)))
model2No  <- sum(p2.No$No[p2.No$Amount==30000]*table(df$Party))/sum((table(df$Party)))
```

According to simulations based on Model 1, at $30,000 per head the average probability of a Yes vote is `r round(model1Yes,digits=2)` while that of a No vote is `r round(model1No,digits=2)`. These are weighted averages by party. At this amount the within-party split between Yes and No probabilities strongly favors a Yes on the Democratic side, as expected based on their higher price for a No:

```{r showBalanceM1, echo=FALSE, eval=TRUE, warning=FALSE, message=FALSE, cache=TRUE}
round(subset(p1.No,Amount==30000)[,c('Yes','No')],digits=2)
```

Model 2 pegs the $30,000 probability of a Yes vote at `r round(model2Yes,digits=2)` and that of a No vote at `r round(model2No,digits=2)` with the intra-party split between Yes and No probabilites as shown below (with Democrats again favoring a Yes):

```{r showBalanceM2, echo=FALSE, eval=TRUE, warning=FALSE, message=FALSE, cache=TRUE}
round(subset(p2.No,Amount==30000)[,c('Yes','No')],digits=2)
```
